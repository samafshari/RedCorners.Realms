...
namespace RedCorners
{
        ...
        where TUpdate : class, new()
    {
        ...
        BenchmarkService GetBenchmarkService() => benchmarkService ?? BenchmarkService.Instance;
        ...
        public RealmTable(Func<Realm> getRealm, BenchmarkService benchmarkService) : this(getRealm)
        {
        ...
        public async Task<TModel> GetAsync(string id)
        {
            await CacheAllIfNeededAsync();
            {
                ...
                TModel result = null;
                if (EnableCaching)
                    using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.GetAsync(Cache)"))
                        cache.TryGetValue(id, out result);
                if (result == null)
                {
                    using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.GetAsync(Realm)"))
                        await Task.Run(() =>
                        {
        ...
        public virtual async Task<List<TModel>> GetAllAsync()
        {
            await CacheAllIfNeededAsync();
            {
                if (EnableCaching && cacheHasAll)
                    using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.GetAllAsync(Cache)"))
                        return cache.Values.ToList();

                List<TModel> results = null;
                using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.GetAllAsync(Realm)"))
                    await Task.Run(() =>
                    {
                        using (var realm = GetRealm())
                        {
        ...
        public async Task<List<TModel>> WhereAsync(Func<TInterface, bool> predicate, Func<IEnumerable<TInterface>, IEnumerable<TInterface>> config)
        {
            await CacheAllIfNeededAsync();
            {
                ...
                await Task.Run(() =>
                {
                    if (EnableCaching && cacheHasAll)
                    {
                        using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.WhereAsync(Cache)"))
                        {
                    ...
                    using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.WhereAsync(Realm)"))
                    using (var realm = GetRealm())
                    {
        ...
        public async Task<TModel> FirstOrDefaultAsync(Func<TInterface, bool> predicate, Func<IEnumerable<TInterface>, IEnumerable<TInterface>> config)
        {
            ...
            await Task.Run(() =>
            {
                if (EnableCaching && cacheHasAll)
                {
                    using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.FirstOrDefaultAsync(Cache)"))
                    {
                        var all = cache.Values
                        .Cast<TInterface>()
                        .Where(predicate);
                ...
                using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.FirstOrDefaultAsync(Realm)"))
                using (var realm = GetRealm())
                {
                    var all = realm.All<TRecord>().Where(predicate);
                    if (config != null) all = config(all);
        ...
        public async Task ForceCacheAllAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    isCachingAll = true;
                    using (GetBenchmarkService().StartBenchmark($"{typeof(TModel)}.ForceCacheAllAsync()"))
                    {
                        using (var realm = GetRealm())
                        {
